...
import { startPlayOver, setUserHighlight, undoPlay, redoPlay, markRepeats } from '../../actions/playActions';

import PlayForm from '../../components/PlayForm/PlayForm';

import '../EditPanel/EditPanel.css';

const PlayPanel = ({interactionsConfig, onStartOver, onColorSelected, onMarkRepeats, canUndo, onUndo, canRedo, onRedo}) => {
    return (
        <div className="edit-panel">
            <div className="edit-panel__title">Play Puzzle</div>
            <PlayForm onStartOver={onStartOver} onColorSelected={onColorSelected(interactionsConfig)} onMarkRepeats={onMarkRepeats} canUndo={canUndo} canRedo={canRedo} onUndo={onUndo} onRedo={onRedo}></PlayForm>
        </div>
    );
};
